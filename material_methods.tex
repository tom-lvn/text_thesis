...
A first CNN classifies fishing vessels into one of sixteen categories \figref{fig:vessel_classification} and 
...
the whole Mediterranean and is thus, quite large. See \Cref{fig:workflow} for an overview of the GFW processing pipeline.
...
\begin{forest}
    [
    Fishing, fill=grayBase, for tree={
        grow=east,
        draw,
        rounded corners,
...
\begin{figure}[htp]
    \begin{subfigure}[b]{0.45\textwidth}
        ...
        \node[anchor=north west, font=\large] at (-6,7.7) {(A)};
...
\end{subfigure}
\bigskip
\caption{A) Conceptualization of space-time dependency as implemented in the Emerging Hotspot Analysis tool. One \textit{neighbourhood  bin}
is defined as the cell itself (darkgreen) plus the cells surrounding it (lightgreen), as well as those cells in the previous time step. 
B) Structure of the space-time cube in ArcGIS\@. Each individual cube corresponds to one \textit{neighbourhood bin}. One \textit{time series bin}
corresponds to the same location over time (red).}
\label{fig:cube}
\end{figure}
